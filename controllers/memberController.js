...
exports.getProfile = async (req, res) => {
  ...
  try {
    ...
    // Admin view: fetch member by query param ?memberId=
    if (req.admin) {
      userId = req.query.memberId;
      if (!userId) {
        const missingId = { success: false, message: "Member ID required for admin" };
        console.log(`[${new Date().toISOString()}] Response to frontend:`, missingId);
        return res.status(400).json(missingId);
      }
    }

    // Member view: fetch own profile from token
    if (req.member) {
      userId = req.member.id;
    }

    if (!userId) {
      const unauthorizedResponse = { success: false, message: "Unauthorized: Invalid token" };
